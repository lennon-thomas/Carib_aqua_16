...
eez<-st_read(dsn=paste(boxdir,"Tonga/tmp/",sep=""),layer="Tonga_eez") 

ext<-extent(-174.2, -173.85, -18.9, -18.55)

Tonga_eez<-readOGR(dsn=paste(boxdir,"Tonga/tmp/",sep=""),layer="Tonga_eez",stringsAsFactors=FALSE)
...
tonga_depth<-raster(paste(boxdir,"Tonga/tmp/tonga_depth.tif",sep=""))
tonga_depth<-crop(depth,Tonga_eez)
...
tonga_depth[tonga_depth<50]<-0

tonga_depth[tonga_depth>100]<-0


tonga_depth[tonga_depth>0]<-1
...
s<-stackApply(s,indices=c(1,1,1),fun=prod,na.rm=FALSE)
...
all_suitable<-disaggregate(all_suitable,fact=10,method='bilinear')

all_suitable<-mask(all_suitable,Tonga_eez,maskvalues=NA,inverse=FALSE)

all_suitable<-mask(all_suitable,Tonga_eez,maskvalues=NA,inverse=FALSE)
test<-crop(all_suitable,extent(Tonga_eez))
test<-resample(test,Tonga_eez)
tm_shape(all_suitable)+
  tm_raster(showNA = FALSE, legend.show = FALSE, palette = c("red"),labels = c("Suitable Areas"),title = "")+
...
plot(all_suitable,col='lightblue', main = 'Suitable Aquaculture Areas');plot(Tonga_eez,add=T)
