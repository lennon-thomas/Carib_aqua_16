...
carib_eez_raster_mask <- mask(carib_eez_raster,carib_depth,progress='text',filename=paste(boxdir,'Suitability/tmp/carib_eez_ocean.tif',sep=""),overwrite=TRUE)
...
plot(MPA_raster)
head(MPA_raster)
MPA_raster<-resample(MPA_raster,carib_depth,progress='text',filename=paste(boxdir,'Suitability/tmp/WRI MPA_1km.tif',sep=""))
...
#Add a land mask
carib_poly_raster<-mask(carib_poly_raster,depth,maskvalues=NA,filename=paste(boxdir,"Suitability/raw/ICUN MPA/MPA poly raster final.tif",sep=""),overwrite=TRUE)
poly<-raster(paste(boxdir,"raw/ICUN MPA/MPA poly raster final.tif",sep=""))
...
#add land mask
mask(points,depth,maskvalue=NA,inverse=FALSE, file=paste(boxdir,"raw/ICUN MPA/MPA point_final.tif",sep=""),overwrite=TRUE)
...
UNEP_MPA<-mask(UNEP_MPA,depth,inverse=FALSE,maskvalue=NA,filename=paste(boxdir,"Suitability/tmp/WDPA MPA",sep=""))
plot(UNEP_MPA,col=c("purple","lightblue"),add=TRUE)

##Combine WRI and WDPA MPA layers
WRI<-raster(paste(boxdir,"Suitability/tmp/MPA_1km.tif",sep=""))
plot(WRI)
WRI[is.na(WRI)]<-0
WRI[WRI>0]<-3
...
writeRaster(final_mpa,paste(boxdir,"Suitability/tmp/final_mpa_layer.tif",sep=""))
...
resample(s_thermo,depth,method="ngb",filename=paste(boxdir,"raw/carib_thermo_1k.nc",sep=""),varname="ttd",overwrite=TRUE)
carib_thermo<-brick(paste(boxdir,"Suitabiliyt/raw/carib_thermo_1k.nc",sep=""))
mask(carib_thermo,depth,maskvalue=NA,filename=paste(boxdir,"Suitability/raw/carib_thermo_final.nc",sep=""),overwrite=TRUE)
...
final_min<-raster(paste(boxdir,"Suitability/tmp/min_thermo.tif")
...
output1<-paste(boxdir,"Suitability/raw/currents/data/tmp/",year,"_1c.nc")
for(i in seq_along(currentlist)){crop(currentlist[[i]],c(19.83333, 380.16667, -80.16667, 80.16667),file=output1[i],overwrite=TRUE)}



currentfiles<-list.files(paste(boxdir'Suitability/raw/currents/data/tmp/'),full.names=T, pattern="nc")
currentlist<-lapply(currentfiles,brick)

output2<-paste(boxdir"Suitability/raw/currents/data/tmp/2c/",year,"_2c.nc")
...
year<-as.vector(c("2005","2006","2007","2008","2009","2010","2011","2012","2013","2014","2015"))
currentfiles<-list.files(paste(boxdir,'Suitability/raw/currents/data/tmp/2c/'),full.names=T, pattern="nc")
currentlist<-lapply(currentfiles,brick)

output3<-paste(paste(boxdir,"Suitability/raw/currents/data/tmp/1k/"),year,"_1km.nc")
...
currentfiles<-list.files(paste(boxdir,'Suitability/raw/currents/data/tmp/1k'),full.names=T, pattern="nc")
currentlist<-lapply(currentfiles,brick)
...
currentfiles<-list.files(paste(boxdir,'Suitability/raw/currents/data/final_v/'),full.names=T,pattern='tif')
currentlist<-lapply(currentfiles,raster)
all<-brick(currentlist)
max_u<-calc(all,fun=function(x){max(abs(x),na.rm=TRUE)})

writeRaster(max_u,paste(boxdir,"Suitability/raw/currents/data/final_v/final/allyears_v.tif"))

###run interpolation here using OA_interpolation.py file.

###Read in interpolated file and add land mask
max_u<-raster(paste(boxdir,"Suitability/raw/currents/data/final_v/final/allyears_v_int.tif"))
plot(max_u)
depth<-raster(paste(boxdir,"Suitability/tmp/carib_depth.tif"))
...
writeRaster(final_max_v,paste(boxdir,"Suitability/raw/currents/data/final_v/final/final_max_int_v.tif"))

cols<-(terrain.colors(250))
png(file=paste(boxdir,"Suitability/raw/currents/data/final_v/final/max_v_final_plot.png"))
plottitle<-("Absolute Maximum Meridional Velocity\n 2006-2015")
plot(final_max_v,col=cols,main=plottitle,legend.args=list(text="m/s",side=3))
dev.off()

currentfiles<-list.files(paste(boxdir('Suitability/raw/currents/data/raw data/',full.names=T, pattern="nc"))
varname="u"
#varname="v"
...
output3<-paste(boxdir,"Suitability/raw/currents.data/tmp/1k/",year,"_1km.nc")
depth<-raster(paste(boxdir,"Suitability/tmp/carib_depth.tif"))
...
output5<-paste(boxdir,"Suitability/raw/currents/data/final_u/",year,"_max.tif")
...
currentfiles<-list.files(paste(boxdir,'Suitability/raw/currents/data/final_u/',full.names=T,pattern='tif'))
currentlist<-lapply(currentfiles,raster)
all<-brick(currentlist)
max_u<-calc(all,fun=function(x){max(abs(x),na.rm=TRUE)})


writeRaster(max_u,paste(boxdir,"Suitability/raw/currents/data/final_u/final/2005_u.tif"))

###run interpolation here using OA_interpolation.py file.

###Read in interpolated file and add land mask
max_u<-raster(paste(boxdir,"Suitability/raw/currents/data/final_u/final/2005_u_int.tif"))
plot(max_u)
depth<-raster(paste(boxdir,"Suitability/tmp/carib_depth.tif"))
final_max_u<-mask(max_u,depth,maskvalue=NA,inverse=FALSE)

plot(final_max_u)
writeRaster(final_max_u,paste(boxdir,"Suitability/raw/currents/data/final_u/final/final_max_int_u.tif"))
cols<-(terrain.colors(250))
png(file=paste(boxdir,"Suitability/raw/currents/data/final_u/final/max_u_final_plot.png"))
...
projectRaster(ships,crs="+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0",filename=paste(boxdir,"Suitability/tmp/ships_proj.tif",sep=""))
ships2<-raster(paste(boxdir,"Suitability/tmp/ships_proj.tif",sep=""))

resample(ships2,world,method="ngb",filename=paste(boxdir,"Suitability/tmp/ship_1k.tif",sep=""))
ship1k<-raster("ship_1k.tif")

crop(ship1k,depth,filename=paste(boxdir,"Suitability/tmp/carib_ship.tif",sep=""))
...
oil2<-raster(paste(boxdir,"Suitability/2013stablelights.tif",sep=""))
...
plot(oil,col="black")
crop(oil, carib_depth,filename=paste(boxdir,"Suitability/tmp/carib_oil.tif",sep=""),overwrite=TRUE)
oil<-raster(paste(boxdir,"tmp/carib_oil.tif",sep=""))

resample(oil,carib_depth,method="ngb",filename=paste(boxdir,"Suitabiliyt/tmp/carib_oil_1k.tif",sep=""),overwrite=TRUE)
oil<-raster(boxdir,"Suitability/tmp/carib_oil_1k.tif",sep="")
oil[is.na(oil)]<-0
...
coral<-readOGR(dsn=paste(boxdir,"Suitability/raw/coral/01_Data",sep=""), layer="14_001_WCMC008_CoralReef2010_v1_3")
carib_depth<-raster(paste(boxdir,"Suitability/tmp/carib_depth.tif",sep=""))
carib_coral<-crop(coral,ext,progress='text',filename=paste(boxdir,'Suitability/tmp/carib_coral.tif',sep=""))
