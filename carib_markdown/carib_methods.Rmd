...
To maintain a steady supply of product, commercial mariculture farms typically stage the stocking of cages so that fish of a harvestable size are available year-round.Cobia take twelve months on average to grow from fingerling (~3 kg) to a harvestable size of 5 - 6 kg [@Holland] . For our cobia farms, we assume that 5 cages are stocked with cobia fingerlings during the first month, and that one cage is stocked with fingerlings each of hte remaining months.  The number of fingerlings stocked in each cage remained constant, and was calculated assuming a conservative harvest density of 15 *$\frac{kg}{m^3}$* ), (or 16,000 individual fish per cage).
...
where $t_min$  is 22 $^\circ$C, t_*max* is 32 $^\circ$C, and max growth occurs at t_*opt* (29 $^\circ$C), and SST is the average monthly SST over the last 10 years for locations, $D$, is the initial stocking density  of each cage, and $S$, the total survival rate is assumed to be 85% [@Benetti2008c]. 
...
Total revenue ($TR$) during each year $t$ for each farm $i$ is calculated as:

$TR_{i,t} = (B_{i,t} * p_t) - B_{i,t} * k_{EEZ}$ 

Where $B$ is the total biomass of cobia at farm $i$ in year $t$, $k$ is the export tax as a function of EEZ, and $p$ is the global price of cobia in year $t$ (see Production Scenarios section for explanation of global price values used in our analyses). Cobia farms in the Caribbean are assumed to be price takers and thus production in year $t$ does not affect price in year $t+1$.
