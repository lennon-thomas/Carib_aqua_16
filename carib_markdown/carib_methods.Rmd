...
#### Costs
The total cost $TC$ of production for each farm $i$ consists of start up costs ($SC$) plus the sum of operating costs ($OC$). Cost parameters were obtained from published literature or from personal communication with industry experts and included some parameters that were fixed across our study region  (Table S3a) and some parameters as a function of EEZ (Table S3b) 
...
We calculate the 10-year net present value (NPV) of farm $i$ using the discount rates defined in the model scenarios ($\delta_{z}$) (Table 5): 
...
Defining appropriate discount rates in economic models is often difficult [REF]. We run our bioeconomic model under two different discount scenarios. In the first scenario, we assume a fixed disocunt rate of 10% across all farms, which is based on the average discount rate of 10.6 % found in a meta-analysis of literature on bioeconomic modelling in aquaculture from the last XX years [@RuizCampo2018]. However, lack of foreign investment due to financial risk has been identfied as a major barrier to aquaculture development in the Caribbean. Foreign investment is often artifact of both political and economic risk in a country, which can be calculated into a single risk score using economic and socioeconomic indicators [@Bhalla2009]. Risk of investment is often facotred in to the decision of selecting an appropariate discount rate when estimating returns on an investmebt. We use economic and socioeconomic indicators for each country to calculate a score representing the risk of foreoing investment for each country. Risk scores are then scaled to EEZ specific discount rates from XX to XX, which represents the range of discount rates found in the literature of bioeconomic modelling of aquaculture [REF].

### Sensitivity Analysis

Feed is well known to account for the large portion of operating costs in aquaculture (REF). Techonology is currently being developed ot improve the efficience of feed used in aquauclutre, which will reduce costs. Because we expect a reduction in feed costs to occur over the next decade, we run a sensitivity analysis to determine the impact on production across the region under a scenario where feed price is reduced by 10% of the current price.
...
pander::pandoc.table(suit_eez_table,
                     ...
                     caption = "Table 1: Estimated suitable area (km^2^ by EEZ)")
```
...
include_graphics(path = paste0(run_dir,'Figures/eez_suitability_map_zoom.png'))
```
