...
We apply an instantaneous mortality rate of 0.024, which we adopt from previous studies that have estimated a total 12 month survival rate of 75% for cobia raised in offshore cages in the Caribbean [@Benetti2008a; @Huang2011]. A cage at site $i$ is harvested when individual fish have grown to a harvestable size of 5 - 6 kg. 
...
Fingerling and feed costs ($F_{i,t}$) include the total cost of fingerlings stocked in month $i$ ($2.58n_{0,i,t}$) plus the total cost of feed. Feed cost is based on a feed price of \$1.68/kg and a tapered feeding strategy in which daily feed usage (30 days per month) corresponds to 3% of total farm biomass ($B_{i,t}$) for the first three months, 2% for months 4-8, and 1% thereafter. 

Farms earn revenue by harvesting market weight cobia, which we assume to be 5 kg. Thus, farms only earn revenue in months where individual fish weight ($w$) reaches 5 kg. In these months, farm revenue is a function of harvest biomass ($B_{i,t}$, kg) and cobia price ($p$), $R_{i,t} = B_{i,t}p$, otherwise $R_{i,t} = 0$. Cobia farms in the Caribbean are assumed to be price takers and thus production in month $t$ does not affect price in month $t+1$. Total farm profit ($\pi$) is the sum of revenues less total farm costs.
...
Where $\delta_{z}$ is the discount rate. Lastly, for comparing with national statistics, NPV is annualized by calculating the corresponding annuity $A_i$ payment:

$$ A_i = \frac{\delta_{z}(NPV)}{1 - (1+\delta_{z})^{-10}}  $$

Production and annuity values for each EEZ are examined under two main scenarios: 1. "Suitable", where farms are developed in all areas that are technically suitable, and 2. "Profitable", where only farms with a positive 10-year NPV at the current price of cobia are developed.

Defining appropriate discount rates in bioeconomic models is often difficult and depends on many factors, particularly the cost of capital and project risk [@Stanley1990]. As a result, we also examine two different discount rate assumptions. First, we assume a fixed discount rate of 10% across all farms, which is based on the average discount rate of 10.6% found in a meta-analysis of published bioeconomic models for aquaculture from the last 25 years  [@RuizCampo2018]. However, lack of foreign investment due to perceived financial risk has been identfied as a major barrier to aquaculture development in the Caribbean [@Halwart2007]. Investment risk can be represented by adjusting the discount rate accordingly when estimating returns on an investment. Therefore, we also examine results using EEZ-specific discount rates that are based on the financial risk of investing in offshore aquaculture in each country. 
...
Accounting for technical, environmental and use conflict constraints, we identified `r prettyNum(main_results[1,2],big.mark=",", digits = 5)` km^2^ of ocean space (`r round(main_results[2,2], digits = 2)`% of our study region) potentially suitable for the development of offshore mariculture (Figure 1). Depth was the largest constraining factor in the suitability analysis, as 97.85% of our study area fell outside the depth range (25 - 100 m) that we considered technically feasible for offshore farm infrastructure. By comparison, our second largest constraining factor was shipping activity, which only excluded 11% of our study area (Table S4). 

There is considerable variability in the amount and distribution of suitable area for offshore mariculture in the Caribbean (Table 1; Figure 1). The Bahamas, Trinidad and Tobago, and Jamaica are the top three countries in terms of suitable area, and the spatial distribution and clustering of suitable sites differs considerably among them (Figure 1). Both The Bahamas and Trinidad and Tobago have more than twice the suitable area of the next largest country with the next largest amount of suitable area (Jamaica; Table 2). The large suitable area in the Bahamas can be attributed to its relatively large EEZ (Table 2; Figure 1). Overall, Trinidad and Tobago had the largest percentage of their EEZ identified as suitable (13.3%), followed by Saba (11.1%) (Table 2). Suitable areas accounted for < 5% of total EEZ area for all other Caribbean islands and no suitable areas were found in Martinque, St. Martin, and Guadeloupe because of the conservation area status of otherwise suitable sites (Table 2).

## Cobia production

Ignoring economic constraints, the Caribbean's potential to produce cobia from mariculture is extremely large, with an approximate annual production from all suitable sites of `r round(main_results[15,2] / 1e6 / 10, digits = 1)` MMT. The median cobia farm occupying a 1-km^2^ site in the Caribbean yields an annual supply and value (annuity) of `r prettyNum(main_results[9,2], big.mark = ",", digits = 5)` MT and \$`r prettyNum(main_results[9,2], big.mark = ",", digits = 5)`, respectively. Not surprisingly, international variability in production is driven by the amount of suitable area within each EEZ - countries with the most suitable area also have the largest production potential (Table 2; Figure 2A). However, countries with largest production potentiall do not neccessarily have the most productive farms (Figure 2). Cobia growth is a function of temperature, and growth rates (kg/month) vary across EEZs, showing a clear seasonal pattern (Figure 3). As a result, the most productive farm sites lie within the EEZs of Jamaica and several small nations in the southeastern Caribbean, such as St. Vincent and the Grenadines, Barbados, St. Lucia, and Trinidad and Tobago (Figure 3). Interstingly, farms in Haiti and the Cayman Islands experience below average growth rates during the peak summer months, likely due to water temperatures approaching $T_{max}$.  

Variability in growth rates, both spatial and seasonal, leads to differences in the length of time required to raise cobia to a market size of 5 kg (Figure S2). While the average farm in the Caribbean complete a harvest cycle in `r median(fcr_results$harvest_length, na.rm = T)` months, harvest cycles range from `r range(fcr_results$harvest_length)[1]` to `r range(fcr_results$harvest_length)[2]` months in duration. Longer harvest cycles affect the economics of cobia production by reducing harvestable biomass (mortality/escapes) and by increasing feed use and other operating costs. Feed conversion ratio (FCR) - calculated as total feed used divided by total harvested biomass - is a measure of how efficiently a farmed animal converts feed into biomass and is one of the main indicators used to compare the sustainability of different animal protein sources (REF). FCRs of cobia farms in our analysis ranged from `r range(fcr_results$fcr_rate)[1]` to `r range(fcr_results$fcr_rate)[2]`, with a median FCR of `r median(fcr_results$fcr_rate, na.rm = T)`. These findings accord with the literature, in which FCRs for cobia range from 1.5-3 . Our findings also support the conclusion that feed accounts for the vast majority of farm operating costs, with the median farm spending `r round(100*main_results[5,3], digits = 0)`% of operating costs on feed. 

When economics are considered, the potential production and/or profitability of offshore cobia mariculture is considerably reduced in several countries relative to when only suitability and temperature-dependent growth are considered (Table 4; Figure 4). In the Bahamas, which has the largest amount of suitable area, average annual production declines by `r round(scenario_compare$supply_diff[scenario_compare$country=='Bahamas'] / 1e6 / 10, digits = 1)` MMT (`r round(scenario_compare$supply_diff_perc[scenario_compare$country=='Bahamas'], digits = 1)`%), while Cuba experiences a decline of `r round(scenario_compare$supply_diff[scenario_compare$country=='Cuba'] / 1e6 / 10, digits = 1)` MMT (`r round(scenario_compare$supply_diff_perc[scenario_compare$country=='Cuba'], digits = 1)`%). The Turks and Caicos and Haiti also both experience small declines (<1%). Overall, the potential annual production for the Caribbean is reduced to `r round(main_results[20,3] / 1e6 / 10, digits = 1)` MMT when only considering supply from profitable farm sites. In terms of value, farms in most countries were profitable and this result was robust to the discount rate (Figure 4). Only the Bahamas and Cuba contained a considerable number of unprofitable farm sites, with the median Bahamian farm unprofitable if assessed with a 10% discount rate. Farm annuity value is tightly clustered around $3 million for many countries, suggesting site selection is less of a concern in these areas. However, farms in Turks and Caicos, Jamaica, the Dominican Republic, Cuba, the Bahamas, and Aruba vary considerably in value and site selection is thus a critical factor for prospective cobia farms (Figure 4). 

# Discussion 

Evaluating and incorporating economic and social factors is a critical step in identifying potential sites for mariculture development that is often overlooked (@Natale2011; @Stelzenmuller2017). Previous studies have found large potential for global mariculture development by identifying suitable area using environmental data (@Kapetsky2013; @Gentry2017), and by estimating biological production rates that could occur in identified suitable area (@Gentry2017). To date, studies that have applied bioeconomic modeling to examine the economic feasibility of mariculture farms have largely been focused on a single farm in a previously specified area [@Kam2003; @Shamshak2009; @Kaiser2010; @Bezerra2016;]. However, the economic feasibility of offshore mariculture in the Caribbean has not been previously evaluated [@Benetti2010]. Our study presents a framework for incorporating biological, environmental, economic, and political data to estimate production and economic potential at a regional scale that can be applied to other regions, and using other species. Our results reveal high potential for cobia offshore mariculture development in the Caribbean region, highlights where in the region culturing of cobia is most productive, and can be used to identify sites within an EEZ that will be most economically profitable for offshore cobia mariculture development.  

The total annual production of 0.4 MMT possible from all suitable areas where farms are economically feasible in the Caribbean is two orders of magnitude larger than total current seafood production in the region (~300,000 MT) and approximately equal to the total annual harvest from the world’s capture fisheries (~80 MMT). Impressively, this output requires just 1.7% of the Caribbean’s marine space, a result similar to that of Gentry et al. (2017) who estimated that current total landings could be produced from 0.015% of global ocean area. In fact, the Caribbean could match its current seafood production using just 178.8 km2 (0.006%) of its marine space. 

Our results indicate that space for offshore development in the Caribbean is not currently a a limiting factor. However, where farms are sited can have a large impact on their realized profitability highlighting the importance of marine spatial planning that incorporates economic factors at both the regional and EEZ spatial scale. In our study, the Bahamas had large potential in terms of suitable area due to its large EEZ, which aligns with conclusions of previous studies [@Patil2016]. However, once economic were incorporated, it was revealed that the majority of the cobia mariculture farms within the Bahamas EEZ would not be profitable. The results of our study suggest that both the production and economic potential of offshore mariculture in the Bahamas may be improved by culturing a species that reach an optimal growth a cooler temperature relative to cobia. Cobia appears to be a species well suited for culture in the warmer waters found in Jamaica’s EEZ and the southeastern Caribbean. 

EEZs with more farm to farm variability in outcomes represent countries where strategic site selection for mariculture development is particularly important. The location of farms located within the EEZs of Cuba, Bahamas, and Turks and Caicos varied on the scale of millions of dollars annually and whether a farm could be economically viable.  As population, tourism, and shipping in the Caribbean are predicted to continue increasing, and new marine industries begin to emerge careful marine spatial planning will become increasingly important to ensure sustainable development of this new frontier [@Patil2016]. In California, comprehensive, coordinated, and strategic MSP applied to theoretical aquaculture development was shown to maximizes objectives across current and emerging sectors while minimizing environmental impacts (Lester et al. 2018).

In our study, the within EEZ variability in profits were largely the result of variability in sea surface temperatures highlighting the importance of carefully considering local oceanographic features when evaluating site location and considering the economic implications of such features. Distance of farm from shipping area , availability of land onshore for processing facilities, and availability of labor force are important additional considerations that may impact the success of our a farm that were not incorporated into our analyses (@CRFM2014). 

Lack aquaculture development in the Caribbean has largely been attributed to lack of availability affordable credit to assist private sector development and lending environments that are not attractive to foreign investors in the literature (@CRFM2014). Risky investment environments are associated with higher discount rates that are notoriously difficult to predict or quantify[@Jin2005]. Investment in offshore mariculture is generally considered a risky investment due to the relative newness of the industry and its unpredictable nature [@Lipton2010]. Our study attempted to account for the risk of foreign investment associated with each country by incorporating political and economic country specific indicators to define country specific discount rate. Due to the large uncertainty associated with these values, we also applied our model using a standard, fixed discount rate. In our study, profits from farms in countries that were associated with a higher investment risk (and discount rate) had lower median annuity values. Although, the discount rate incorporating risk did not mean a previously profitable EEZ was no longer profitable, the lowered associated annuity values would likely be unattractive to investors. In Haiti for example, a country associated with high investment risk due to the countries low GDP and high political corruption, the risk associated discount factor translated to a loss in median farm level annuity close to 0.5 million dollars, a value that could likely sway an investor to opportunities that are less risky.

Policy for aquaculture development varies considerably across the Caribbean, and will largely play a role in how and where aquaculture is developed [@Price2014; @Corbin2017]. Countries that aquaculture specific legislation, and policies in place to promote the development of aquaculture such as fiscal interventions are more likely to attract foreign investors. For example, all aquaculture related products imported to the Dominican Republic are exempt from import tax [REF]. To ensure any development occurs sustainably it is essential for countries to have clear policies in place that include an evaluation on the environmental impacts of farms and general guidelines on where development can occur. Having clear policies and process for aquaculture development is important as overly lengthy or confusing processes for acquiring permits to begin development can be a deterrent for potential investors. For example, Snapperfarm, a small-scale offshore mariculture farm originally based in Puerto Rico had difficulty obtaining permits to expand due to uncertainty about environmental impacts, and so they recently moved to Panama [@Price2014]. 

The results of our model are largely dependent on several key assumptions: the depth range that is technically feasible for offshore development, the current price of cobia and feed, and the market price of cobia. Technology in the offshore aquaculture infrastructure industry is rapidly developing, and it is likely that in the near future installing farm infrastructure at greater maximum depths will be technically and economically feasible. Caribbean island countries have large EEZ relative to their land areas and depth was the largest constraining factor in our suitability assessment. As the suitable depth range for this industry expands, the area suitable for offshore aquaculture development will become larger. As this occurs, it will be essential to consider additional costs associated with farms at deeper depths such as distance from shore, more costly maintenance, etc. prior to development to ensure that farms at greater depths are economically viable. 

Data on current price of cobia feed is difficult to obtain. Values reported in the literature range from US$ 1.64/kg [@Bezerra2016] to US$ 1.80/kg (Sclodnick2014) thus we feel our assumed value of XX is reasonable and conservative. Additionally, feed costs in our model accounted for on average 95% of  operating costs, which is similar to values previously reported in the literature for cobia farms (@Huang2011). Feed conversion ratio (FCR) - calculated as total feed used divided by total harvested biomass - is a measure of how efficiently a farmed animal converts feed into biomass and is one of the main indicators used to compare the sustainability of different animal protein sources (REF). Our calculated FCR values ranged from XX to XX, which is similar to what has been reported for cobia in the literature. However, cheaper, lower quality feed may be available, which could lower feed costs, but increase the FCR potentially having a large impact on farm production and profitability[Ref]. The cost and quality of feed available for aquaculture generally varies by region[Ref]. Our model assumed that the quality of feed available in the Caribbean would be similar to what has been reported for cobia in the scientific literature and did not account for any costs that may be related to importing and shipping feeds from far away.

Our model assumes that cobia produced in the Caribbean would be for export market and sold at the current global price for cobia of $8.62/kg. As shown in Figure SX our results are largely sensitive to these results and a $ /kg in feed price is associated with a XX decrease in supply as a large number of farms are no longer profitable at a lower price. If the current estimated biomass of cobia were produced in the Caribbean, it is likely that the market would become flooded and the price would be reduced. In 20XX, global farm produced cobia increased by XX % and was associated with a XX drop in $/kg. Global dynamics of cobia supply and demand should be carefully considered before any rapid development of cobia takes place. Cobia produced for domestic markets could help increase regional food security and reduce reliance on seafood imports, which have increased in recent years [@Nguyen2010]. One consideration for moving forward with offshore mariculture development in the Caribbean could be to diversity the species being cultured and choses species that are best suited in terms of optimal growth associated temperature for different regions.

Sustainable aquaculture development can be greatly assisted by adopting a planned approach to development [@Krause2015]. This study offers the first comprehensive look at the production potential of offshore mariculture in the Caribbean region in terms of profits and biomass and we recommend the results of our study be used to help guide and plan offshore mariculture development in the Caribbean. To avoid market saturation and increase potential profits we recommend the framework presented here be used to explore the potential for other offshore aquaculture mariculture species in the Caribbean. 
