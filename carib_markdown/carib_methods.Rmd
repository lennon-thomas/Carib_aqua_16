...
## Model Scenarios 
*(would be helpful to talk through how we want to define our base case and whether everything else should just fall under sensitivity)*

First, we examine average annual production under a 'maximum production scenario', where the annual production cobia is estimated assuming all farms were developmed in all cells that were identified as suitable without economic considerations. 

We also examine production and 10 year NPV under each of the following scenarios assuming only cells that are profitable after 10 years are developed:

* High feed cost, country specific discount rates
* High feed cost, discount rate fixed across all countries at 15%
* Low feed cost, country spcific discount rates
* Low feed cost, discount rate fixed across all countries at 15%

High feed cost refers to the current cost of feed (1.65/kg), and low feed cost assumes that advances in feed technology have reduced feed costs by 10% of current feed costs (XX ). Country specific discout rates refer to the discount rates estimated based on the countries foreign investment risk. Price of cobia for the four scenarios is fixed at the current global price of $8.64/kg.  

Production and 10 year NPV are examined within EEZ, across EEZs, and at the regional level under each of the 4 scenarios. Estimated production under each scenario is also compared to current seafood production (wildfisheries capture + aquaculture) and current seafood consumption in the Caribbean region. 

## Sensitivity analyses 
*(maybe we just big one 'base run from Table xx and then the rest of the sccenarios fall under Sensitivity analyses?)*
...
* Demand curve/cobia price
* Discount rates
* Fixed stocking density (described below)
...
```
*will add current seafood production and consumption to this figure*
```{r, fig.cap = "Estimated total production of cobia (mt) for our study area under each of our 5 model scenarios"}
...
```{r fig.cap = "Average growth rate by month for each EEZ"}
