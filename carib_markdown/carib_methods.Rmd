...
To estimate production, we assume a conservative fixed farm design, per 1 km^2^ site, across our study region. SeaStation cages are typically configured using a mooring system that includes a grid, anchor, and mooring lines secured at varying distances from the cages [@Fredriksson_2004; @Xu2015]. The cages are held in position by the mooring system at depths of 15-20 m below the surface[@Loverich2010]. We develop a hypothetical farm design for a 1 km^2^ cell using the total total cage volume to farm area ratio of offshore mariculture farms currently operating off the coast of Kona, Hawaii [@Sims] and the Gulf of Maine [@Fredrisson2004]. Our hypothetical farm has 16 SeaStation cages (each 6,400 m^3^) configured in two eight-cell grid mooring systems, a total cage volume of 102,400 m^3^ per individual farm. The infrastructure of the farm has a footprint of  ~ 0.48 km^2^, which meets the guidelines issued in NOAA's Fishery Management Plan (FMP) for offshore aquaculture development in the Gulf of Mexico that specifies the total mariculture farm area should be twice the size of the total area occupied by farm infrastructure [@GMFMCGulfofMexicoFisheryManagementCouncil2009].
...
For a farm with a 12 month average harvest cycle ($\bar{t}_i=12$), $n^*$ is 25,600.
...
#### Costs
The total cost $TC$ of production for each farm $i$ consists of start up costs ($SC$) plus the sum of operating costs ($OC$). Cost parameters were obtained from published literature or from personal communication with industry experts. Some parameters were fixed across our study region (Table S3a) while others are a function of EEZ (Table S3b). 

$$ TC_i = SC_i + \displaystyle\sum_{t=1}^T OC_{i,t} $$

Costs of aquaculture infrastructure and installation increase with depth and distance from shore [@Beverage2004]. Start-up costs are therefore include initial capital expenditures ($E$) and installation costs ($I$), both as a function of average site depth in meters $m$, and EEZ-specific permit and/or lease cost $P$:

$$ SC_i =  E_i(m_i) + I_i(m_i) + P_{z} $$

Initial capital expenditures ($E$) include the cost of 16 SeaStation fish cages (\$269,701 each) plus the cost of one 16-m-long support vessel and two 7-m-long motorboats. Installation costs include the labor costs for installation of the culture system. A 10% increase in cage and installation costs are assinged to sites with depths greater than 50 m [@Lester2017]. Operating costs are organized in two main categories; farm labor and maintenance costs as a function of site depth and EEZ-specific hourly salary and fuel costs ($L_{i,t}(s_z,d_i,g_z)$), and costs associated with purchasing and feeding stocked fish $F_{i,t}$. 
...
pander::pandoc.table(eez_suit_table,
                     big.mark = ',',
                     col.names = c('Country', 'Total EEZ area (km^2)', 'Suitable area (km^2)','% suitable'),
                     round = c(0,0,0,2),
                     digits = c(0,6,4,2),
                      split.tables = Inf,
...
### Suitable Farm Production and NPV

If economic constraints are ignored and all suitable farm sites are developed regardless of profitability, the Caribbean's potential to produce cobia from mariculture is extremely large, with an approximate annual production of XXX MT (XXX% of total current seafood production in the region). Variability between average annual production of EEZs is largely driven by the amount of area that was identified as suitable within each EEZ (Figure 2). The Bahamas,  largest area suitable for mariculture (and most farms), has an estimated average annual production of XXXX mt and is the largest contributed ot regional supply (XX%).  

```{r , fig.cap = "Figure 2. Estimated total production of cobia (mt) using if all suitable areas are developed as offshore mariculture farms "}
include_graphics(path = paste0(run_dir,'Figures/total_prod_map.png'))
```

## Bioeconomic Model Results

When economics are considered, regional supply is substantially reduced relative to estimated supply when only available are and temperature-depenedent growth are considered.	This difference in production is largely due to feed costs, which account for at least XX% of total costs across all farms. Assuming the current global price of cobia and current feed price, the estimated supply for the region is estimated at XX (Figure 3). If the current price of feed is reduced by 10%, regional supply increases by XX% (Figure 3). Total regional supply decreases quickly as the price for cobia decreases under both feed price scenarios. No farms are profitable in the region if the price of cobia drops below 0 (Figure 3).


```{r, fig.cap="Figure 3. Cobia supply curves under current feed cost (feed price index = 1) and a 10% reduction in feed cost (feed price index = 0.9)."}
knitr::include_graphics(path = paste0(run_dir, 'Figures/carib_supply_curves.png'))
```

The interaction between temperature driven growth rates and feed costs affects the variablity of average annual production  and total 10 year NPV across EEZs. This is especially illustrated in the Bahamas which has a large area suitable for aquaculture but a long average harvest cycle across farms in the EEZ due to cooler waters, making the majority of farms unprofitable (Figure 4). Farms that faill within Trinidad and Tobago conribute XX% to the regional supply and collectively have an estimated 10 year NPV of XXX, the highest 10 year NPV hOnly xx out of xx countries identified with suitable area for aquaculture have farms that are profitable under current feed prices. However, when feed price is reduced by 10%, farms in Dominica, Aruba, and XX become profitable.

**INSERT SUMMARY TABLE OF ALL THREE SCENARIO RESULTS**

. This is especially illustrated in the Bahamas which has a large area suitable for aquaculture but a long average harvest cycle across farms in the EEZ due to cooler waters, making the majority of farms unprofitable.


At the farm level, the median NPV of a cobia farm in the Caribbean was `r prettyNum(main_results[8,3], big.mark = ",")`, driven primarily by the large number of farms in Trinidad and Tobago and high NPV farms in several other countries (Figure X). 
...
Feed is well known to account for the large portion of operating costs in aquaculture (REF), and our findings support this conclusion (`r round(100*main_results[5,3], digits = 1)`% of operating costs). However, given that economic forces are driving promising advances in feed technology, we also explored results if current feed costs (\$1.65/kg) are reduced by 10%. A reduction in feed costs by 10% would increase cobia production (supply) by XXX. No farms in the Caribbean would be profitable if price for cobia drops below $8/kg at current feed costs.



A reduction in feed costs would increase production and NPV...

```{r fig.height = 5, fig.width = 4, fig.cap = "Total production (mt) (top) and 10 year NPV ($) (bottom) of farms that are profitable over a 10 year period at current feed prices and when cost of feed is reduced by 10%"}
include_graphics(path = paste0(run_dir,'Figures/econ_npv_prod_map.png'))
```
...
```{r fig.cap = "Figure S1. Areas suitable for offshore mariculutre in the Caribbean"}
...
suit_var_table <- suit_var_table %>%
  dplyr::select(-1)

pander::pandoc.table(suit_var_table,
                     big.mark = ',',
                     col.names = c('Layer',"Suitable area (km^2)","% of Study Area"),
                     round = 2,
                     digits = 2,
                      split.tables = Inf,
