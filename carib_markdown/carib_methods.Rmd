...
table3<-read.csv("table_3.csv",check.names=FALSE)
table4<-read.csv("table4.csv",check.names=FALSE)
#table5<-read.csv(paste0(boxdir,"results/Suitability/suit_by_variable.csv"),check.names=FALSE)
table6<- read_csv(file = paste0(boxdir,filename = "results/Suitability/Suit_df_summary.csv"))
...
#### Discount Rate (need to update and add methods used to convert to discount rate)
...
## Bioeconomic Scenarios (would be helpful to talk through how we want to define our base case and whether everything else should just fall under sensitivity)

We examine average annual production under 2 scenarios: 1) a maximum production scenario, where the annual production cobia is estimated assuming all farms were developmed in all cells that were identified as suitable without economic considerations, and (2) total production of farms that only includes farms that are profitable after 10 years, assuming the current global price of cobia.he total production under both scenarios both production scenario is compared to current seafood production (wildfisheries capture + aquaculture) and current seafood consumption in the Caribbean region. Production both within and across EEZs are also compared.

The 10 year NPV under scenario #2 is examined for profitable all profitable cells at the level farms, EEZs and Caribbean region. The 10 year NPVs are also examined and compared for each of sensitivity scenarios described below. 

## Sensitivity analyses (maybe we just big one 'base run from Table xx and then the rest of the sccenarios fall under Sensitivity analyses?)

Due the uncertainty of future market conditions, economic status of countries,impacts new techonological developments, and to identify the current major hurdles to the development of offshore aquaculture in the Caribbean, we apply our bioeconomic model under several different scenarios to examine NPV of the region and each EEZ over a 10 year period by performing a suite of sensitivity analyses. using different assumptions about: feed costs, discount rates, intial stocking densities. 10 year NPV of different model runs are compared.
...
### Low Feed cost scenario

We assume techonology of feed development of advances, reducing current feed costs by 10% of the current assumed value.

### Discount rates

We assume a fixed discount rate of 14% across all EEZs.
...
```{r fig.3}
include_graphics(path = paste0(boxdir,'results/Suitabilty/all_suitable.png'))
...
There is considerable variability in the amount of area suitable for offshore mariculture in Caribbean nations (Table 1). Both the Bahamas and Trinadad and Tobago have more than twice the suitable area than the country with the next largest amound of suitable area (Jamaica) 
...
## Production Scenarios
 ...
 If economic constraints are ignored and all suitable farm sites are developed, the Caribbean's potential to produce cobia from mariculture is extremely large, XXX mt annually. 
...
include_graphics(path = paste0(run_dir,'Figures/Caribbean_avg_growth.png'))
```

As a result of the variability in growth rates acros the Caribbean, farms vary in the length of time require to raise cobia to a market size of 5 kg, with the median farm requiring X months to complete a harvest cycle (Figure XX).

```{r}

#include_graphics(path = paste0(run_dir,'Figures/harv_length_hist.png'))
...
For our base case scenario (country specific risk, current global price, high feed cost), 17 EEZs were profitable after 10 years, and Trinidad and Tobago had the highest NPV, followed by Jamaica.

```{r}
knitr::include_graphics(path = paste0(run_dir, 'Figures/risk_high_feed_map.png'))
```

```{r}
knitr::include_graphics(path = paste0(run_dir, 'Figures/Caribbean_npv_cell.png'))
```


```{r, fig.cap="Figure XX. Distribution of farm-level net present value (NPV) by EEZ. Fill color represents the total NPV for each country."}
knitr::include_graphics(path = paste0(run_dir, 'Figures/npv_scenarios_map.png'))
```
...
knitr::include_graphics(path = paste0(run_dir, 'Figures/eez_suitable.png'))
...
knitr::include_graphics(path = paste0(run_dir, 'Figures/eez_npv_cell.png'))
