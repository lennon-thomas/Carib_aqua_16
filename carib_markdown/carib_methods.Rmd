...
} else boxdir <- '../../../Box Sync/Carib_aqua_16/'
...
3. Offshore aquaculture is an emerging approach to mariculture where farms are some distance offshore  
    ...
    * Overview of offshore aquaculture worldwide (very brief)  
...
Start-up costs ($SC$) include initial capital expenditures as a function of average site depth $E_i(depth_i)$, installation costs as a function of distance from port and EEZ ($I_i(dist_i,EEZ)$), and EEZ-specific permit and/or lease cost $P_{EEZ}$:
...
Where $B_{i,t}$ is the total biomass of cobia at farm $i$ in year $t$, $k$ is the export tax as a function of EEZ, and $p$ is the global price of cobia in year $t$ (see Production Scenarios section for explanation of global price values used in our analyses). Cobia farms in the Caribbean are assumed to be price takers and thus production in year $t$ does not affect price in year $t+1$.
...
* Fixed Price Scenarios:
    * Maximum production scenario: All suitable cells are developed (and estimate what the global price would have to be to make all suitable cells profitable)
    * Current market scenario: Total production assuming current market price (insert value and reference) of cobia and costs of production

* Demand Curve Scenario: We assume a global demand curve for cobia and estimate total production assuming the price of cobia in year t + 1 adjusts to Caribbean supply in year t
...
1. Existing practices capacity limit - Ecological production limit is determined by identifying the highest aquaculture per unit area of EEZ worldwide
