...
Individual cobia growth at month $t$ for site $i$ ($G_{t,i}$) is estimated using the TPC for all 120 months in the SST timeseries. The 10 year ($y$) average individual growth ($\bar{G}_{t,i} $) at site $i$ is then calculated for each calendar month $t$:

$$\bar{G}_{t,i} = \frac{\sum_{y=1}^{10} G_{y,t,i}}{10} $$

## Bioeconomic Model
We model future offshore aquaculture potential in the Caribbean by coupling site specific estimates of average monthly cobia growth with a detailed economic model parameterized with values from the literature, technical reports, and personal communication with producers of commercial aquaculture technology (e.g. Innovasea).

### Growth Model
Individual cobia growth at month $t$ for site $i$ ($G_{t,i}$) is modeled using the previously calculated 10 year ($y$) average individual growth ($\bar{G}_{t,i} $) at site $i$ for each calendar month $t$. Total farm biomass ($B_{t,i}$) is a function is the initial stocking density of each cage, and $S$, the total survival rate is assumed to be 85% [@Benetti2008c]. 
