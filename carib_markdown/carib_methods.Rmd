...
# Main results - profitable farms, current cobia and feed price, and 10% discount rate
...
pander::pandoc.table(risk_table,
                     caption = 'Investment risk scores and discount rates by country. A risk score of 1.5 (US Virgin Islands) is used as a baseline for scaling discount rates between 10%-20%')
```
...
```{r sum total suitable area}
total_s_area<-sum(suit_eez_table$`Suitable area (km^2^)`,na.rm = TRUE)
perc_total_area<-total_s_area/(sum(suit_eez_table$`Total EEZ Area (km^2^)`))*100
```

Based on the factors we considered and thresholds we defined, there is  `r prettyNum(total_s_area,big.mark = "", digits = 2)`  km^2^, of area, or `r prettyNum(perc_total_area, big.mark = ",", digits =2)` % our study region, potentially suitable for the development of offshore mariculture, Depth was the largest constraining factor in the suitability analysis, `r 100-(round(suit_var_table[7,3]), digits = 2)`% of our study area fell outside the depth range ( 25 - 100 m) considered techinically feasible for offshore mariculture farm infrastructure. By comparison, our second largest constriaining factor in the suitability analysiswas shipping activity, which only excludedr `r 100- (round(100 - suit_var_table[6,3], digits = 2))`% of our study area as suitable for offshore mariculture development (Table S4). 

There is considerable variability in the amount of area potentially suitable for  offshore mariculture in Caribbean nations (Table XX). Both the Bahamas and Trinadad and Tobago have more than twice the suitable area than the country with the next largest amound of suitable area (Jamaica) (Table 1; Figure 1). No potenitally suitable areas were found in  Martinque, St. Martin, and Guadeloupebecause of the conservation status of waters falling within their EEZs. The large potentially suitable area in the Bahamas can be attributed to its relatively large EEZ (Table 1; Figure 1). Overall, Trinidad and Tobago had the largest portion of their EEZ identified as potentially suitable of offshore mariculture development (13.31%) (Figure 1), followed by Saba (11.07 %). Potentially suitable areas acoounted for < 5% of totall EEZ area for all other island countries.

```{r table1, results='asis'}
#rownames( Table 1 suit_eez_table) <- NULL
#suit_eez_table <- suit_eez_table %>%
 # dplyr::select(-X)
pander::pandoc.table(suit_eez_table,
                     big.mark = ',',
