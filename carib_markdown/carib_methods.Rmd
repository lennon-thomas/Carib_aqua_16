...
Cobia are harvested in months where $w_{t} >= 5$ (under the assumption harvest occurs at the beginning of the month) and, while $g <= g_i$, farms are restocked with $n_0$.
...
Start-up costs, which include initial capital expenditures ($E$) and installation costs ($I$) (see 'Costs' section in Supplementary Information for a breakdown of start-up costs), vary as functions of average site depth in meters $m$:
...
Fingerling and feed costs ($F$) include the total cost of stocked fingerlings ($\$1.50 * n_{0,i,t}$) plus the total cost of feed. Cost of feed is based on a feed price of \$2.50/kg and a tapered feeding strategy in which daily feed usage (30 days per month) corresponds to 3% of total farm biomass ($B_{i,t}$) for the first three months, 2% for months 4-8, and 1% thereafter. For  $F_{i,t}$. For details on how labor costs are calculated see 'Costs' section of Supplementary Information. 

Farms earn revenue $R$ by harvesting market weight (5 kg) cobia. Thus, farms only earn revenue in months where individual fish weight ($w_t$) reaches or exceeds 5 kg, otherwise $R_{i,t} = 0$. Revenue is a function of harvested biomass ($B_{i,t}$, kg) and cobia price ($p$), where $R_{i,t} = B_{i,t}p$. We assume the current global market price of $8.62/kg.
...
$$ A_i = \frac{\delta_{z}(NPV)}{1 - (1+\delta_{z})^{-10}} (9) $$
...
Lack of foreign investment due to perceived financial risk has been identified as a major barrier to aquaculture development in the Caribbean [@Nations2007]. Foreign investment risk reflects both political and economic risks in a country, which can be calculated into a single risk score using economic and socioeconomic indicators [@Bhalla1983]. Investment risk can be represented by adjusting the discount rate accordingly when estimating returns on an investment [@Stanley1990]. We apply the Foreign Investment Risk Matrix (FIRM) method developed by Bhalla [-@Bhalla1983] and modified by McGowan [@McGowanJr2009], which utilizes  political and  economic  risk variables that are readily available for most countries to calculate an investment risk score for each country (See 'Risk Adjusted Discount Rate' section in Supplemenatry Information and Tables S4-S5). Risk scores are the scaled to EEZ-specific discount rates (Table 1) between 10% and 25%, which aligns with the range of discount rates found in the literature of bioeconomic modelling of aquaculture [@RuizCampo2018].
...
The datasets generated during and/or analysed during the current study and code are available in the XX git-hub repository, [PERSISTENT WEB LINK TO DATASETS].
