...
3.	Overview of cobia biology and cobia farming (this section should make it clear why SST was not a necessary layer in our suitability assessment , but was important to consider in our growth model).
...
### Growth Model
Individual cobia growth at month $t$ for site $i$ ($G_{i,t}$) is modeled using the previously calculated 10 year ($y$) average individual growth ($\bar{G}_{i,t} $) at site $i$ for calendar month $t$. Fingerlings are stocked at an initial weight of 0.15 grams and fish weight ($w$) in month $t$ at site $i$ is calculated as the cumulative growth since the stocking month ($t_s$):

$$ w_{t,i} = \sum_{t_s}^{t} \bar{G}_{i,t} $$
...
$$ B_{t,i} =  \displaystyle\sum_{c = 1}^{C} w_{c,t,i} n_{s,i} - n_{s,i} (1-e)^{0.024~t} $$
...
Costs of aquaculture infrastructure and installation increase with depth and distance from shore[@Beverage2004]. Start-up costs are therefore modeled to include initial capital expenditures ($E$) as a function of average site depth in meters $m$, installation costs ($I$) as a function of distance from port ($d$) and EEZ ($z$), and EEZ-specific permit and/or lease cost $P$:

$$ SC_i =  E_i(m_i) + I_i(d_i,z) + P_{z} $$

$$ E_i = \displaystyle\sum $$

$$ I_i = $$

A 10% increase in installation cost are assinged to sites with depths greater than 50 m [@Lester2017] or locations further than 25 nm from shore (@Kapetsky2013).

Operating costs are organized in two main categories; costs associated with farm labor and maintenance, and costs associated with purchasing and feeding stocked fish. Labor and maintenance costs ($L$) at site $i$ in month $t$ are modeled as follows: 

$$ L_{i,t} = hls_z + \frac{d_i}{b}srg_z + d_i b g_zet $$

Where $h$ is the hours per laborer required at site $i$, $l$ is the labor (number of workers) required to service a farm; $s$ is the EEZ-specific hourly salary; $d_i$ is distance from shore (km); $b$ is average boat speed to farm sites; $r$ is the number of trips required per month for running the farm; $g$ is the cost of fuel as a function of EEZ; $e$ is the average fuel efficiency of the vessel; $f$ is the total cost of fingerlings and $h$ is the cost of feed in month $t$ for a individual farm.
