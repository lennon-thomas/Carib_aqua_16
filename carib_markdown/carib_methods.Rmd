...
Marine aquaculture development in the Caribbean has been identified as an avenue to stimulate local economies, increase employment opportunities, and improve seafood supply and food security. However, when located close to shore, aquaculture’s environmental impacts can jeopardize sensitive habitats (e.g., mangroves, coral reefs) and competition for space can conflict with fisheries, tourism, and other uses. Farming further from shore is an option for alleviating these stresses. Using high-resolution environmental and economic data, we develop a spatial bio-economic model to identify suitable areas throughout the Caribbean for offshore cobia aquaculture (*Rachycentron canadum*), a species with considerable market potential, and estimate yields and profits for cobia farms under different economic scenarios. We find that approximately `r prettyNum(main_results[1,2],big.mark=",", digits = 2)` km^2^ (`r round(main_results[2,2], digits = 2)`%  is technically feasible for cobia aquaculture, with the potential to produce  `r round(main_results[20,2] / 1e6 / 10, digits = 1)` million metric tons (MMT) of seafood annually, an amount roughly equal to the total global production from capture fisheries. Given our estimates of production costs and cobia price, ~ `r prettyNum(main_results[1,3],big.mark=",", digits = 5)`  km^2^  (`r round(main_results[2,3], digits = 2)`%) is economically viable for cobia farming, with the potential to produce  `r round(main_results[20,3] / 1e6 / 10, digits = 1)` MMT of cobia annually. When accounting for foreign investment risk, these estimates drop slightly and some countries look far less favorable for aquaculture development. The variability of farm-scale production and value, and thus the importance of within-EEZ site selection, differed across countries, demonstrating the value of spatial models for decision-making at different spatial scales. 
...
Fingerling and feed costs ($F$) include the total cost of fingerlings stocked in month $i$ ($2.58n_{0,i,t}$) plus the total cost of feed. Feed cost is based on a feed price of \$1.68/kg and using a tapered feeding strategy in which daily feed usage (30 days per month) corresponds to 3% of total farm biomass ($B_{i,t}$) for the first three months, 2% for months 4-8, and 1% thereafter. 
...
Production and annuity values for each EEZ are calculated for two main scenarios: 1. "Suitable", where farms are developed in all areas that meet the criteria used in our suitability assessment, and 2. "Profitable", where only farms with a positive 10-year NPV at the current price of cobia are developed.
...
Lastly, our model assumes that cobia produced in the Caribbean would be sold as an export product at the current global price for cobia of $8.62/kg,  and does not account for Caribbean production influencing global prices, an assumption which would certainly not hold under significant levels of development. For example, in 20XX, global farm produced cobia increased by XX % and was associated with a  drop in X/kg [REF]. Global dynamics of cobia supply and demand should be carefully considered before any rapid development of cobia takes place. One option for moving forward with offshore mariculture development in the Caribbean could be to diversify the species being cultured and choose species that are best suited in terms of optimal temperature-dependent growth for different regions.
