...
4.	Overview of cobia biology and cobia farming (this section should make it clear why SST was not a necessary layer in our suitability assessment , but was important to consider in our growth model).
  ...
  * @Bezerra2016 looked at economic viablity of both nearshore and offshore cobia mariculture in Brazil. There offshore industrial farm had a total volume of 38,568 m^3. initial investment was 1.5 million $USD and avereage payback 3.88 years. had 17 employees.

5. Review of other studies that have done similar analyses (suitability or economic viability and what they lacked (leading in to what are study contributes))
...
Individual cobia growth at month $t$ for site $i$ ($G_{t,i}$) is estimated using the TPC for all 120 months in the SST timeseries. The 10 year ($y$) average individual growth per month ($\bar{G}_{t,i} $) at site $i$ is then calculated for each calendar month $t$:

$$\bar{G}_{t,i} = \frac{\sum_{y=1}^{10} G_{y,t,i}}{10} $$

#### Optimal Stocking Density
The number of completed harvest cycles ($h$), wherein a cobia is raised from an initial stocking weight of 0.15 g to a final harvest weight of 5 kg, is calculated for each site by dividing the cumulative individual growth by 5 kg. 

$$h_i = \frac{\sum_{t=1}^{120} G_{t,i}}{5~kg} $$  

The average number of months per harvest cycle ($\bar{h_i}$) is then calculated as $\bar{h_i} = \frac{120}{h_i}$. Given $\bar{h_i}$, a target harvest density of $XX \frac{kg}{m^3}$, and an annual survival rate of 85%, the optimal number of fingerlings to stock can be estimated for each site:

** Insert equation optimal initial stocking density **

## Bioeconomic Model
SST is the average monthly SST over the last 10 years for locations, $D$, is the initial stocking density  of each cage, and $S$, the total survival rate is assumed to be 85% [@Benetti2008c]. 

Annual production ($B_t$) at each farm is then calculated as: 

$B_t = c * \displaystyle\sum_{m=1}^{12} B_m$

Where $c$ is the number of cages per farm and $B_m$ is the monthly total biomass per cage.
...
Costs of infrastructure increase with depth [@Beverage2004] =Start-up costs ($SC$) include initial capital expenditures as a function of average site depth $E_i(depth_i)$, installation costs as a function of distance from port and EEZ ($I_i(dist_i,EEZ)$), and EEZ-specific permit and/or lease cost $P_{EEZ}$:

$SC_{i,t=1} =  E_i(depth_i) + I_i(dist_i,EEZ) + P_{EEZ}$

A 10% increase in installation cost were assinged to sites that were in depths >50 m [@Lester2017] or > 25 nm from shore (@Kapetsky2013).
...
Where $H_i$ is the hours required at site $i$ as a function of wave exposure; $w$ is the number of workers required to service a farm; $d$ is the EEZ-specific hourly wage; $dist_i$ is distance from shore (km); $b$ is average boat speed to farm sites; $r$ is the number of trips required per month for running the farm; $g$ is the cost of fuel as a function of EEZ; $e$ is the average fuel efficiency of the vessel; $f$ is the fixed cost of fingerlings for a individual farm in year $t$; and $h$ is the cost of feed in year $t$ for a individual farm.
...
Where $B$ is the total biomass of cobia at farm $i$ in year $t$, $k$ is the export tax as a function of EEZ, and $p$ is the global price of cobia in year $t$.  Cobia farms in the Caribbean are assumed to be price takers and thus production in year $t$ does not affect price in year $t+1$. We also explore a scenario with a downward demand curve where Caribbean cobia production does affect price (see *Production Scenarios* section).
