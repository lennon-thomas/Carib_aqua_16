...
raster_eez<-raster(paste(boxdir,"Suitability/tmp/carib_eez_ocean.tif",sep=""))
non_countries<-c(81,103,112,263,54,55,62,63,98,108,109,125,126,128,271,272,273,274,108,164,129,130,131,132,133,135,139,163,167,255,221,120)
for (i in 1:length(non_countries)){
  raster_eez[raster_eez==non_countries[i]]<-0
}
raster_eez[raster_eez==0]<-NA
eez<-raster_eez
eez[eez>0]<-1
writeRaster(eez,paste(boxdir,"Suitability/final/suitable_eez.tif",sep=""))

png(file=paste(boxdir,"Suitability/plots/suitable_eez.png",sep=""))
plottitle<-("suitable_eez")
plot(eez,col="lightblue")
dev.off()
...
land<-raster(paste(boxdir,"Suitability/tmp/carib_depth.tif",sep=""))
carib_depth<-mask(carib_depth,land,inverse=FALSE,maskvalues=NA,filename=paste(boxdir,"Suitability/final/suitable_depth.tif",sep=""),overwrite=TRUE)
plot(carib_depth)
...
land<-raster(paste(boxdir,"Suitability/tmp/carib_depth.tif",sep=""))
suitableMPA<-mask(MPA,land,maskvalues=NA,inverse=FALSE,filename=paste(boxdir,"Suitability/final/suitable_MPA.tif",sep=""),overwrite=TRUE)

png(file=paste(boxdir,"Suitability/plots/suitable_mpa.png",sep=""))
plottitle<-("MPA areas")
plot(carib_depth,main=plottitle,col="red")
dev.off()

# #Assigns cells either a 1 (< 1 m/s )  or 0 based on current velocity
u<-raster(paste(boxdir,"Suitability/tmp/final_max_int_u.tif",sep=""))
v<-raster(paste(boxdir,"Suitability/tmp/final_max_int_v.tif",sep=""))
s<-stack(u,v)
current<-calc(s,fun=function(x){max(abs(x),na.rm=FALSE)})
current<-mask(current,eez,inverse=FALSE,maskvalues=1)
current<-mask(current,land,maskvalues=NA,inverse=FALSE)
current[current>1]<-0
current[current==1]<-0
...
ships<-mask(ships,land,maskvalues=NA,inverse=FALSE)
ships[ships>0]<-1
...
png(file=paste(boxdir,"Suitability/plots/suitable_oik.png",sep=""))
...
coral<-mask(coral,eez,inverse=FALSE,maskvalues=1)
coral[coral==0]<-1
coral[coral==2]<-0

s = stack(carib_depth,suitableMPA,current,ships,eez,oil)#,coral)
s<-stackApply(s,indices=c(1,1,1,1,1,1,1),fun=prod,na.rm=FALSE)
all<-mask(s,land,maskvalues=NA,inverse=FALSE,filename=paste(boxdir,"Suitability/results/suitable_areas.tif",sep=""),overwrite=TRUE)
#library(raster)
all<-raster("C:/Users/Lennon Thomas/Desktop/Carib_aqua_16/Suitability/final/Suitable areas.tif")
all[all==0]<-NA
saveRDS(all,paste(data_dir,"final/suitable_areas.rds",sep=""))
